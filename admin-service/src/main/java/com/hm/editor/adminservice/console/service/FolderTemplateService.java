...
public class FolderTemplateService {
  ...
  @Autowired FolderTemplateRepository folderTemplateRepository;
  ...
  public ApiResult getBasTemplateList(EmrBaseFolder folder) {
    List<EmrBaseTemplate> templates = folderTemplateRepository.getListByFolderId(folder.get_id());
    templates.forEach(t -> t.initIDStr());
  ...
  public List<Map> templateDs(String name) {
    ...
    List<EmrBaseTemplate> templates =
        folderTemplateRepository.getListByCri(Criteria.where("templateName").is(name));
    List<String> dsSet;
    if (templates.isEmpty() || (dsSet = templates.get(0).getDsSet()) == null || dsSet.isEmpty()) {
