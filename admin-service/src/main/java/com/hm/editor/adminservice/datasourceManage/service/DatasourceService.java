...
public class DatasourceService {
  ...
  public boolean delDs(String id) {
    ...
    Map d =
        commonRepository.findOne(
            datasourceColName, Criteria.where("_id").is(DataUtil.str2ObjectId(id)));
    commonService.delData(DataUtil.str2ObjectId(id), datasourceColName);
    if (d != null && d.get("code") != null) {
      commonRepository.delData(
          Criteria.where("code").is(d.get("code")), ContantUtil.DS_REFRENCE_COLLECTION_NAME);
      commonRepository.delData(
          Criteria.where("refCode").is(d.get("code")), ContantUtil.DS_REFRENCE_COLLECTION_NAME);
    }
    return true;
  }
  ...
  public Map getDs(String text, int pageNo, int pageSize) {
    ...
    Map d =
        commonService.pageData(datasourceColName, text, pageNo, pageSize, "code", "code", "name");
  ...
  public String generalCode(String name) {
    ...
    if (code != null && !code.toString().isEmpty()) {
      index =
          Integer.parseInt(code.toString().replace(DEFINED_DS_PRE, "").replace(".", "").trim()) + 1;
    }
