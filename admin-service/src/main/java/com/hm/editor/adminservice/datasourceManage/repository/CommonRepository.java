...
public class CommonRepository {
  ...
  public void inserts(List<Map> d, String colName) {
    BulkOperations bulkOperations =
        mongoTemplate.bulkOps(BulkOperations.BulkMode.UNORDERED, colName);
  ...
  public List<Map> agg(List<AggregationOperation> aggs, String name) {
    Aggregation _agg =
        newAggregation(aggs).withOptions(newAggregationOptions().allowDiskUse(true).build());
  ...
  public Object maxVal(String name, Criteria criteria, String fieldName) {
    Aggregation agg =
        newAggregation(match(criteria), sort(Sort.Direction.DESC, fieldName), limit(1))
            .withOptions(newAggregationOptions().allowDiskUse(true).build());
