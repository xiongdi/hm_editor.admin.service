...
public class FileUtil {
  ...
  public static void generateCSVFile(List<List<Object>> objects, String fileName, boolean append) {
    try {
      ...
      CsvWriter writer =
          cn.hutool.core.text.csv.CsvUtil.getWriter(file, StandardCharsets.UTF_8, append);
  ...
  public static void transcodeUTF8_GB2312(String source, String target) {
    try (BufferedReader read =
        ...
        BufferedWriter write =
            new BufferedWriter(new OutputStreamWriter(new FileOutputStream(target), "GB2312"))) {
  ...
  public static void transcodeUTF8_code(String source, String target, String code) {
    try (BufferedReader read =
            new BufferedReader(new InputStreamReader(new FileInputStream(source), "UTF-8"));
        BufferedWriter write =
            new BufferedWriter(new OutputStreamWriter(new FileOutputStream(target), code))) {
      String item = read.readLine();
      while (item != null) {
        write.write(item + "\r\n");
        item = read.readLine();
    ...
    for (List<List<Object>> _data : dataList) {
      ...
      while (index < _data.size()) {
        ...
        for (Object h : excelHead) {
          ...
          tempData.put(
              h.toString(), h.toString().equals("createDate") ? new Date() : _o == null ? "" : _o);
        }
